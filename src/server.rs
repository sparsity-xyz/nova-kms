...
async fn get_data(
    ...
    let s = state.read().await;
    let auth =
        crate::auth::authenticate_app(&headers, &s.config, &s.registry, &s.nonce_store).await?;

    let key = params
        .get("key")
        .ok_or_else(|| KmsError::ValidationError("Missing key parameter".to_string()))?;

    let store = s.store.get_namespace(auth.app_id).await;
...
async fn put_data(
    ...
    let key = body
        .get("key")
        .and_then(|v| v.as_str())
        .ok_or_else(|| KmsError::ValidationError("Missing key".to_string()))?;
    let val_b64 = body
        .get("value")
        .and_then(|v| v.as_str())
        .ok_or_else(|| KmsError::ValidationError("Missing value".to_string()))?;

    let plaintext = b64
        .decode(val_b64)
        .map_err(|_| KmsError::ValidationError("Invalid base64".to_string()))?;
