...
pub async fn authenticate_app(
    ...
    if !config.in_enclave {
        let wallet_str = headers.get("x-tee-wallet")
            .and_then(|h| h.to_str().ok())
            .ok_or_else(|| KmsError::Unauthorized("Missing x-tee-wallet header in dev mode".to_string()))?;
            
        let wallet = Address::from_str(wallet_str)
            .map_err(|_| KmsError::Unauthorized("Invalid x-tee-wallet format".to_string()))?;

        let instance = registry.nova_registry.getInstanceByWallet(wallet).call().await
            .map_err(|e| KmsError::InternalError(format!("Registry error: {}", e)))?._0;
        ...
        if uint_instance_id == 0 {
            return Err(KmsError::Unauthorized("Instance not found in registry".to_string()));
        }
    ...
    let nonce = headers.get("t-nonce").and_then(|h| h.to_str().ok())
        .ok_or_else(|| KmsError::Unauthorized("Missing t-nonce".to_string()))?;
    let timestamp_str = headers.get("t-timestamp").and_then(|h| h.to_str().ok())
        .ok_or_else(|| KmsError::Unauthorized("Missing t-timestamp".to_string()))?;
    let signature_str = headers.get("t-signature").and_then(|h| h.to_str().ok())
        .ok_or_else(|| KmsError::Unauthorized("Missing t-signature".to_string()))?;
    let requester_url = headers.get("t-requester-url").and_then(|h| h.to_str().ok())
    ...
    if now > timestamp + config.pop_timeout_seconds || timestamp > now + 30 {
        return Err(KmsError::Unauthorized("Timestamp expired or too far in future".to_string()));
    }

    if !nonce_store.check_and_store(nonce, now, config.pop_timeout_seconds).await {
    ...
    let recovered_address = signature.recover_address_from_msg(payload.as_bytes())
        .map_err(|_| KmsError::Unauthorized("Failed to recover address from signature".to_string()))?;

    let instance = registry.nova_registry.getInstanceByWallet(recovered_address).call().await
        .map_err(|e| KmsError::InternalError(format!("Registry error: {}", e)))?._0;
    ...
    let app = registry.nova_registry.getApp(instance.appId).call().await
        .map_err(|e| KmsError::InternalError(format!("Registry error: {}", e)))?._0;
