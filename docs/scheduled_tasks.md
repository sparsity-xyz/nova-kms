...
1.  **Peer Discovery (`peer_cache.refresh`)**
    *   Fetches the latest list of `ACTIVE` KMS instances from the `NovaAppRegistry`.
    *   Updates the local peer cache.
    *   **Gate check**: If the local node (`self`) is not in the list of active operators, the node marks itself as `503 Unavailable` and stops processing for this tick.
...
4.  **Data Synchronization (`push_deltas`)**
    ...
    *   **Push**: Sends recent data deltas (App/Version/Instance updates) to all healthy peers.
...
| :--- | :--- | :--- |
| `KMS_NODE_TICK_SECONDS` | `5` | core heartbeat interval |
| `SYNC_INTERVAL_SECONDS` | `60` | interval for pushing data deltas to peers |
| `PEER_CACHE_TTL_SECONDS` | `60` | (internal) max age of peer cache before forced refresh |
